...
Friend Class rsclsProducts2
	...
	'=============================================================
	'Name: rsclsProducts2 (a RecordSet class)
	'
	...
	' rsclsProducts2 Foreign Key RecordSet classes
	Public rsSuppliers As ADODB.Recordset
	Public rsCategories As ADODB.Recordset
	
	'Select Parameter properties for the RecordSet class.
	...
	' UPGRADE_INFO (#0511): The 'CategoriesCategoryName' member is referenced only by members that haven't found to be used in the current project/solution.
		Get
	...
	' UPGRADE_INFO (#0511): The 'SuppliersCompanyName' member is referenced only by members that haven't found to be used in the current project/solution.
		Get
	...
	Private Sub Class_GetDataMember(ByRef DataMember As String, ByRef Data As Object)
		...
		Case "products_fetch"
			...
			If Not mbDataInitialized Then 
				de1.rsProducts_Fetch.DataMember = ""
				
				If SaveMode = EnumSaveMode.adImmediate Then 
					de1.rsProducts_Fetch.LockType = ADODB.LockTypeEnum.adLockOptimistic
				Else
					de1.rsProducts_Fetch.LockType = ADODB.LockTypeEnum.adLockBatchOptimistic
				End If
				
				de1.Products_Fetch(Me.Products_FetchProductName)
				'Disconnect the RecordSet class to allow (1) stored procedure access
				'  and (2) control over database update timing.
				de1.rsProducts_Fetch.ActiveConnection = Nothing
				...
				For Each oField In de1.rsProducts_Fetch.Fields
				...
				If de1.rsProducts_Fetch.RecordCount > 0 Then 
					de1.rsProducts_Fetch.MoveFirst()
				End If
				...
				While Not de1.rsProducts_Fetch.EOF
					i = -1
					sLastFieldName = ""
					For Each oField In de1.rsProducts_Fetch.Fields
						...
						Case "supplierid"
							...
							Do While Not de1.rsSuppliers.EOF
								...
								If de1.rsSuppliers.Fields("supplierid").Value = de1.rsProducts_Fetch.Fields("supplierid").Value Then 
						...
						Case "categoryid"
							...
							Do While Not de1.rsCategories.EOF
								'If the Select Foreign Key values equal the Foreign Key command Primary Key values, update the Foreign Key Descriptor field.
								If de1.rsCategories.Fields("categoryid").Value = de1.rsProducts_Fetch.Fields("categoryid").Value Then 
									'Load the vValue Array element with the Foreign Key Descriptor value.
						...
						vValue(i) = de1.rsProducts_Fetch.Fields(oField.Name).Value
...
errMove:
	...
	' UPGRADE_INFO (#0511): The 'MovePrevious' member is referenced only by members that haven't found to be used in the current project/solution.
		'=============================================================
		'Name: MovePrevious
		'
		'Author: Microsoft Data Object Wizard
	...
	Public Function ValidateData() As Boolean
		...
		If IsNull6(rs.Fields("ProductID").Value) Then 
			MsgBox6("The field ' ProductID ' cannot be null.")
			Exit Function
		End If
...
errUpdateBatch:
	...
	Public Sub Update()
	' UPGRADE_INFO (#0511): The 'Update' member is referenced only by members that haven't found to be used in the current project/solution.
		'=============================================================
		'Name: Update
		'
		'Author: Microsoft Data Object Wizard
...
errUpdate:
	...
	' UPGRADE_INFO (#0511): The 'UpdateCategoriesCategoryName' member is referenced only by members that haven't found to be used in the current project/solution.
		'=============================================================
	...
	' UPGRADE_INFO (#0511): The 'UpdateSuppliersCompanyName' member is referenced only by members that haven't found to be used in the current project/solution.
		'=============================================================
