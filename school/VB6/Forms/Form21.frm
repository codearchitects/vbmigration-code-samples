...
Begin VB.Form Form21 
   ...
   Caption         =   "Print Option For Population report"
   ClientHeight    =   1245
   ClientLeft      =   45
   ClientTop       =   330
   ClientWidth     =   4110
...
Do While Not rs_lv.EOF
    If Check1.Value = 1 Then
        rs_tmp.AddNew
            ...
            rs_prnt_list.Filter = "SchoolYear ='" & DataCombo1.Text & "' And LevelNo =" & rs_lv.Fields(0) & " And Sex ='Male'"
            rs_tmp.Fields(1) = rs_prnt_list.RecordCount
            rs_prnt_list.Filter = "SchoolYear ='" & DataCombo1.Text & "' And LevelNo =" & rs_lv.Fields(0) & " And Sex ='Female'"
            rs_tmp.Fields(2) = rs_prnt_list.RecordCount
            rs_prnt_list.Filter = "SchoolYear ='" & DataCombo1.Text & "' And LevelNo =" & rs_lv.Fields(0)
            rs_tmp.Fields(3) = rs_prnt_list.RecordCount
        rs_tmp.Update
    Else
        rs_tmp.AddNew
            rs_tmp.Fields(0) = rs_lv.Fields(1)
            rs_prnt_list.Filter = "Status <>'Drop' And SchoolYear ='" & DataCombo1.Text & "' And LevelNo =" & rs_lv.Fields(0) & " And Sex ='Male'"
            rs_tmp.Fields(1) = rs_prnt_list.RecordCount
            rs_prnt_list.Filter = "Status <>'Drop' And SchoolYear ='" & DataCombo1.Text & "' And LevelNo =" & rs_lv.Fields(0) & " And Sex ='Female'"
            rs_tmp.Fields(2) = rs_prnt_list.RecordCount
            rs_prnt_list.Filter = "Status <>'Drop' And SchoolYear ='" & DataCombo1.Text & "' And LevelNo =" & rs_lv.Fields(0)
            rs_tmp.Fields(3) = rs_prnt_list.RecordCount
        rs_tmp.Update
    End If
